...
class psc_ba_env(gym.Env):
    def __init__(self, adaptation_duration, data_ID, max_ba_flow, gut_deconj_freq_co_multiplier, gut_biotr_freq_CA_multiplier):
        ...
        self.action_amplitude = {0:0.05, 1:0.025, 2:0.025, 3:0.025, 4:0.05} # 0,4: mulplicative; 1,2,3: additive
        ...
        self.NORMAL_DIGESTION = 995341.3282085225 # daily small intestine bile acid exposure in healthy controls: rk45_sol[-1440:,6:12].sum()
        self.NORMAL_LIVER_BA = 684045.577289905 # daily liver bile acid exposure in healthy controls: rk45_sol[-1440:,:3].sum()
    ...
    def quantify_param_deviation(self):
        ...
        Then it takes (np.log(0.1) - np.log(0.823)) / np.log(1-0.05) = 40 steps to get there
        See that 0.823 * ((1-0.05)**40) = 0.1
        Roughly each param is allowed to change at most 40 steps, so deviation is bounded by 40 * 5 params = 200
        '''
        deviation = 0
    ...
    def step(self, action):
        ...
        if self.violate_boundary():
            ...
            return self.current_state, reward, done, False, {"ODE_time":self.t_span_rk45[self.round-1][1], "ODE_traj":sol.y,
